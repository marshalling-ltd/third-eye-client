/*
 * third-eye-platform
 *
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 0.1.0
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct JobModel {
    #[serde(rename = "concurrency")]
    pub concurrency: uuid::Uuid,
    #[serde(rename = "created_at")]
    pub created_at: String,
    #[serde(rename = "data", deserialize_with = "Option::deserialize")]
    pub data: Option<serde_json::Value>,
    #[serde(rename = "deleted_at", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub deleted_at: Option<Option<String>>,
    #[serde(rename = "id")]
    pub id: uuid::Uuid,
    #[serde(rename = "job_type")]
    pub job_type: models::JobType,
    #[serde(rename = "status")]
    pub status: models::JobStatus,
    #[serde(rename = "updated_at")]
    pub updated_at: String,
}

impl JobModel {
    pub fn new(concurrency: uuid::Uuid, created_at: String, data: Option<serde_json::Value>, id: uuid::Uuid, job_type: models::JobType, status: models::JobStatus, updated_at: String) -> JobModel {
        JobModel {
            concurrency,
            created_at,
            data,
            deleted_at: None,
            id,
            job_type,
            status,
            updated_at,
        }
    }
}

